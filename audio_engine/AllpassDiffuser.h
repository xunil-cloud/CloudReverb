...
public:
    ...
    bool GetModulationEnabled() { return filters[0]->ModulationEnabled; }
    ...
    double *GetOutput() { return filters[Stages - 1]->GetOutput(); }
    ...
    void SetModRate(double rate)
    {
        ...
        for (size_t i = 0; i < filters.size(); i++)
            filters[i]->ModRate =
                rate * (0.85 + 0.3 * seedValues[MaxStageCount * 2 + i]) / samplerate;
    }
