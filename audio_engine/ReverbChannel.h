...
public:
    ...
    void Process(double *input, int sampleCount)
    {
        ...
        for (int i = 0; i < len; i++)
        {
            outBuffer[i] = dryOut * input[i] + predelayOut * predelayOutput[i] +
                           earlyOut * earlyOutStage[i] + lineOut * tempBuffer[i];
        }
    }
...
private:
    ...
    void UpdateLines()
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            if (delaySamples <
                modAmount +
                    2) // when the delay is set really short, and the modulation is very high
                delaySamples =
                    modAmount + 2; // the mod could actually take the delay time negative, prevent
                                   // that! -- provide 2 extra sample as margin of safety

            auto dbAfter1Iteration = delaySamples / lineDecaySamples *
    ...
    double Ms2Samples(double value) { return value / 1000.0 * samplerate; }
};
